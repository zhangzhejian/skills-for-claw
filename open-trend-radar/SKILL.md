...
在本 skill 目录下运行。脚本入口是 shell：用 `curl` 拉取页面，再用系统自带 `perl` 解析固定 DOM，不依赖 Python 或第三方 Python 包。

```bash
sh scripts/fetch_open_trend_radar.sh --format json
```
...
sh scripts/fetch_open_trend_radar.sh --category 大模型与AI --format md
sh scripts/fetch_open_trend_radar.sh --category 0 --format json

# 限制每个分组输出数量
sh scripts/fetch_open_trend_radar.sh --top 5 --format md

# 保存原始 HTML 方便排查 DOM 变化
sh scripts/fetch_open_trend_radar.sh --save-html /tmp/open-trend-radar.html

# 指定页面 URL；hash 可有可无
sh scripts/fetch_open_trend_radar.sh --url 'https://open-trend-radar.test.rd.ai/#tab-0'
```
...
- 回答“数据如何获取”时，说明这是静态 HTML 内嵌数据，可用 `curl` 拉页面后按 CSS selector/固定 DOM 解析。
